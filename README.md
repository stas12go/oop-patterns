...
```
docs/        учебный план, шпаргалки, разборы
patterns/    реализации, сгруппированные по категориям (GoF)
  Creational/
  Structural/
  Behavioral/
tests/       интеграционные тесты, которые не принадлежат одному паттерну
```
...
patterns/<Категория>/<Паттерн>/
  ...
  src/        реализация
  tests/      тесты, лежат рядом с реализацией
...
composer install
composer test
php patterns/<Категория>/<Паттерн>/Example.php
```
